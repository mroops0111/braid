...
export interface DefineAgentPluginInput {
   ...
   * Defaults to a permissive object so authors don't have to wire one
   * just to satisfy the `Plugin.configSchema` contract.
   */
 ...
 *
 * Unlike storage / source-loader, the binding is constructed
 * synchronously because the actual subprocess spawn happens lazily
 * inside the returned `AgentBinding`. The builder still validates
 * `kind` and supplies a permissive default `configSchema` so the
 * registry can serialise the plugin without the author wiring an
 * empty object themselves.
 */
