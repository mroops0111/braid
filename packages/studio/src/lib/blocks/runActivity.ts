 ...
 *
 * An answer takes minutes to settle,
 * and the blocks only start arriving near the end,
 * so without this the surface is blank for most of the run.
 * The stream carries the work the whole time, it was simply not shown.
 */
export interface RunActivity {
   ...
   *
   * Not enumerated.
   * Which tools an agent has, and what it calls them, belongs to its binding,
   * and naming one agent's here would leave every other agent's work uncounted,
   * on a surface supposed to be neutral about which one is running.
   */
...
// so counting it again here would report the same output twice.
...
export function summariseActivity(events: readonly SkillEvent[]): RunActivity {
  ...
  for (const event of events) {
    // A message still being typed accounts for the run as well as a whole one,
    // and sooner, which is the only reason this line is here at all.
