...
/**
 * The workspace a block belongs to,
 * for the few renderers that must ask the server something about it.
 * Null outside a provider,
 * so a block still renders as plain content instead of throwing.
 */
 ...
 *
 * The audience decides the depth, not which conclusions exist,
 * so a reader who does not want line numbers still sees every finding.
 * `full` is the default,
 ...
 *
 * A run showing its working draws the slice it built,
 * which is part existing graph and part what it is asking for.
 * Only the first half is in the snapshot,
 * so a renderer drawing the whole slice previews these on top of it.
 * Empty outside a provider,
 * where a block describes the graph as it stands and nothing is pending.
 */
