 ...
 * root registers this against `PluginRegistry`; `composeFsApp` resolves the
 * active agent kind from its own options and builds one server-wide binding,
 * rather than `import`-ing `ClaudeCodeAgentBinding` directly. Other agents
 * (anthropic-api, cursor, ollama, codex, ...) register the same way under a
 * different `kind`.
 */
