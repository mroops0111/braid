...
function side(...provenances: readonly ('graph' | 'agent-read')[]): FindingSide {
  return {
    summary: 'A side.',
...
describe('evidenceSupport', () => {
  ...
  it('reads corroborated when a side mixes a graph citation with its own reading', () => {
    expect(evidenceSupport([side('graph', 'agent-read'), side('graph')])).toBe('corroborated')
  })

  it('reads partial when one side rests only on what the run read itself', () => {
    expect(evidenceSupport([side('graph'), side('agent-read')])).toBe('partial')
  })
  ...
  it('lets the weakest side decide rather than the strongest', () => {
    expect(evidenceSupport([side('graph', 'graph'), side('agent-read')])).toBe('partial')
  })
})
