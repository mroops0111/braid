 ...
 *
 * Kept apart from the model repository on purpose.
 * A vector is derived from the node's text,
 * the way an index is derived from a corpus,
 * so it is rebuilt rather than restored,
 * and it never enters the versioned model file.
 */
