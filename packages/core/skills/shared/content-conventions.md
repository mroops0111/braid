...
- **No code identifiers in prose** (file paths, line numbers, type ids). Those go in `metadata.sourceReferences`, or in a `show_evidence` block where the surface draws them as locations a reader can open, never in the human-facing string itself. **Graph node ids are the one exception**, and only in the token form `@node:<id>`, which Studio renders as a live tag. See `reference-syntax.md` for the grammar and for which fields accept it.
