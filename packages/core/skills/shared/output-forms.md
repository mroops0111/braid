...
`$BRAID_UNATTENDED` says who is reading.

**Unset**, somebody asked and is waiting. Write the answer out as markdown: the answer first, then the sources behind each claim with their locations, then any disagreement you found between them. That is the whole of your output, so do not add a summary of it at the end. A reader who has just read the answer is not served by reading it again in miniature.

**`true`**, a batch or a reactor cycle is driving you and nobody is watching. Write only the summary your skill's own Output section describes. A full answer here is paid for once and read by no one.
