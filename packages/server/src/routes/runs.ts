...
export function createRunsRouter(deps: RunsRouterDeps): Hono {
  ...
  //
  // Replaying history over SSE pins a connection for something that is not a
  // stream, and a browser allows only a handful per host, so a few finished
  // runs are enough to stall every request after them. A finished run has
  // nothing left to tail, so it is a plain fetch.
