 ...
 *
 * A run either raises a question and stops,
 * or says there is nothing to ask and proposes.
 * Told only in a prompt, that held about half the time.
 * A run would model most of a spec, submit it,
 * and only then notice the thing it could not decide,
 * leaving a reviewer to approve a change the answer might overturn.
 *
 * Mutual exclusion alone would be worse rather than better.
 * Whichever call came first would win,
 * so a run proposing before it noticed its doubt would have the doubt refused,
 * and the ambiguity would be lost rather than raised.
 * The gate therefore makes proposing conditional on having already decided.
 * A run says there is nothing to clarify before it may propose,
 * which forces the decision to the point where it is still cheap to make.
 *
 * Keyed by run, never by conversation.
 * Answering a clarification starts a new run that continues the same session,
 * and that run must be free to propose,
 * which is the whole point of resuming it.
 *
 * Unattended runs are not gated at all.
 * A batch that applies its own output wants both,
 * the proposal for coverage and the clarification so the doubt stays visible,
 * and refusing one of them would be refusing the mode.
 */
export class RunOutputGate {
   ...
   *
   * Fails open for a run this process never saw open,
   * which is what a restart leaves behind.
   * Losing a declaration must not wedge a run,
   * because how many times a run may submit is settled from its records.
   */
