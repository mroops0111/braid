...
import type { AbsolutePath, AgentBindingDescriptor, AgentEffort, OntologyId, OutputForm, SkillCategory, StorageKind, WorkspaceId } from '@braidhq/schema'
...
export async function composeFsAppWithRegistry(
  ...
  const skillRunner = new SubprocessSkillRunner({
    ...
    ...(uvxBin
      ? { coreGateway: { specUrlFor: (category: SkillCategory, form: OutputForm) => `${loopbackApiUrl}/openapi/runs/${category}/${form}/openapi.json`, uvxBin } }
      : {}),
    referenceDirs: [
