...
The shared concept document for `@braidhq/ontology-ddd`. Every DDD-resident skill (`ddd:extract`, `ddd:clarify`, `ddd:model`) consults this file before authoring nodes / edges, so the vocabulary, wiring rules, and authoring conventions live in one place instead of being duplicated in each Procedure.
