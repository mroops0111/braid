...
import { workspaceAddCommand, workspaceListCommand } from './commands/workspace.js'
...
cli
  .command('workspace add <path>', 'Register an existing workspace with a running server')
  .option('--api <url>', 'Server URL (default http://localhost:4321)', { default: 'http://localhost:4321' })
  .action(async (path: string, flags: { api: string }) => {
    await workspaceAddCommand({ rootPath: path, apiUrl: flags.api })
  })
