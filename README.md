...
[![CI](https://github.com/mroops0111/telos/actions/workflows/ci.yml/badge.svg)](https://github.com/mroops0111/telos/actions/workflows/ci.yml)
...
Intent ↔ Code alignment framework. Treat **code** as the source of truth for *what* your system does, **intent** (PRDs, RFCs, tickets) as the source of truth for *why*, and let Telos extract, merge, and HITL-validate both into a single canonical knowledge graph the whole product team can query.
...
- **Pluggable ontology.** Bring your own domain vocabulary. The default DDD ontology (`@telos/ontology-ddd`) ships boundedContext / aggregate / command / event / rule; swap or extend for a different domain.
...
**Pre-1.0, building in public.** APIs, schemas, and the CLI surface will break between minor versions until 1.0. No `@telos/*` packages are on npm yet; clone the repo and run it locally. File issues, follow the milestone board, expect rough edges.
...
In the workspace's **Skills** tab pick `telos-extract` and click Run. The skill spawns a Claude Code subprocess against your source paths and streams events back to Studio. Output is a stack of Proposals in the **Proposals** tab.
...
Per-tier details (which packages run where, how `@telos/server` embeds vs. runs standalone, how sources sync) live in [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md).
...
| Package | Role | Status |
|---|---|---|
| `@telos/schema` | Zod schemas and TS types for the domain model. Wire format shared by core, server, studio, sdk, cli, plugins. | Stable shape, pre-1.0 |
| `@telos/core` | Domain entities (Workspace / Model / Proposal / ClarifyTicket / Run) + application services + infrastructure ports. | Pre-1.0 |
| `@telos/sdk` | Plugin SDK. `defineSource` / `defineOntology` / `defineAgent`, etc. | Pre-1.0 |
| `@telos/server` | REST API on Hono. Mounts routes for workspaces / proposals / runs / clarify / ontology / events (SSE). | Pre-1.0 |
| `@telos/storage-kuzu` | Embedded graph storage adapter. Zero-infra alternative to Neo4j. | Pre-1.0 |
| `@telos/source-loader-git` | Source loader plugin: clone a remote repo, sync via fetch + reset. | Pre-1.0 |
| `@telos/source-loader-gdrive` | Source loader plugin: pull text and image files from a Drive folder. | Pre-1.0 |
| `@telos/studio` | Vite + React web UI. Workspaces, Skills, Proposals, Graph. Tauri-ready. | Pre-1.0 |
| `@telos/cli` | Command-line entry. | Stub, not usable yet |
| `@telos/desktop` | Tauri 2 shell that bundles `@telos/server` as a sidecar and serves `@telos/studio`. | Planned |
| `@telos/ontology-ddd` | Default ontology plugin: boundedContext / aggregate / command / query / event / rule. | Pre-1.0 |

## Writing Plugins

Three plugin contracts, all behind `@telos/sdk`:
...
**Source loader.** Pull intent or code into a workspace's source directory. The loader is just an async function returning a list of relative file paths; Telos handles dedup, diff, and propagation. See [`packages/source-loader-git/`](packages/source-loader-git/) and [`packages/source-loader-gdrive/`](packages/source-loader-gdrive/).

**Skill.** A `SKILL.md` markdown file describing how Claude Code should walk your sources and emit proposals. No TypeScript required. See [`packages/core/skills/`](packages/core/skills/) for the built-in skills (`telos-extract`, `telos-clarify`, …).
